  ...
  return {
    ...
    resolve: {
      alias: {
        ...
        'firebase/app': path.resolve(path.dirname(fileURLToPath(import.meta.url)), './src/firebase.ts'),
        'firebase/auth': path.resolve(path.dirname(fileURLToPath(import.meta.url)), './src/firebase.ts'),
        'firebase/firestore': path.resolve(path.dirname(fileURLToPath(import.meta.url)), './src/firebase.ts'),
      },
    },
    build: {
      outDir: 'dist',
    ...
    server: {
      ...
      allowedHosts: ['ivanbobkin5-create-cal-time-127d.twc1.net'],
    },
  };
});
