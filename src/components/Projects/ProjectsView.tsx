  ...
  useEffect(() => {
    ...
    const fetchEmployees = async () => {
      try {
        ...
        // Also merge employees from companyData if present
        if (Array.isArray(companyData?.employees)) {
          companyData.employees.forEach((emp: any) => {
            const uid = emp.uid || emp.id;
            if (uid && !employees.some(e => e.uid === uid)) {
              employees.push({ uid, ...emp });
            }
          });
        }
        ...
        if (currentUser && (currentUser.uid || currentUser.id)) {
          ...
          if (existingIdx >= 0) {
            employees[existingIdx] = {
              ...employees[existingIdx],
              photoURL: employees[existingIdx].photoURL || cAvatar,
    ...
    fetchEmployees();
  }, [companyId, companyData?.employees, currentUser]);
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-6 animate-in fade-in duration-200">
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="absolute -top-[2px] -left-[2px] -right-[2px] h-2 bg-gradient-to-r from-indigo-500 via-purple-500 to-indigo-600 rounded-t-2xl" />
                        
                        <div className="p-3.5 md:p-4 pt-5 md:pt-5 flex-1 flex flex-col justify-between">
                          <div>
                            {/* Header Row */}
                            <div className="flex items-start justify-between gap-3 mb-2.5">
                              <div className="flex items-center gap-2.5 min-w-0">
                                <div className="w-9 h-9 rounded-xl bg-indigo-50 text-indigo-600 flex items-center justify-center flex-shrink-0 font-bold shadow-inner">
                                ...
                                <div className="min-w-0">
                                  <div className="flex items-center gap-1.5 flex-wrap mb-0.5">
                                    ...
                                    <span className="px-2 py-0.5 rounded-md text-[10px] font-bold bg-indigo-100 text-indigo-700 flex items-center gap-1 flex-shrink-0">
                                      <Layers className="w-2.5 h-2.5" /> Комплект ({subProjects.length})
                                    </span>
                                    {set.contractNumber && (
                                      <span className="px-2 py-0.5 rounded-md text-[10px] font-semibold bg-gray-100 text-gray-700 flex-shrink-0">
                                        № {set.contractNumber}
                                      </span>
                                    )}
                                    {setStatus === "sent" && (
                                  ...
                                  <div className="flex items-center gap-3 text-[11px] text-gray-500 flex-wrap">
                                    ...
                                    {(() => {
                                      const isCurrentUser = set.createdBy === userId || (currentUser && (set.createdBy === currentUser.uid || set.createdBy === currentUser.id));
                                      const setAvatarLetter = (displayManagerName?.charAt(0) || "U").toUpperCase();
                                      const setAvatarUrl = (isCurrentUser ? (currentUser?.photoURL || currentUser?.avatarUrl || currentUser?.photoUrl || currentUser?.photo) : null) || employee?.avatarUrl || employee?.photoURL || employee?.photoUrl || employee?.avatar || employee?.photo || set.data?.createdByPhoto || set.createdByPhoto;
                                      return (
                                        <span className="flex items-center gap-1.5">
                                          <div className="w-4.5 h-4.5 rounded-full overflow-hidden bg-gray-200 flex items-center justify-center text-[9px] font-bold text-gray-600 shrink-0 ring-1 ring-gray-300/60">
                                            {setAvatarUrl ? (
                                              <img src={setAvatarUrl} alt={displayManagerName} className="w-full h-full object-cover" referrerPolicy="no-referrer" onError={(e) => { (e.currentTarget as HTMLElement).style.display = 'none'; }} />
                                            ) : (
                                              setAvatarLetter
                                            )}
                                          </div>
                                          <span className="font-semibold text-gray-700">{displayManagerName}</span>
                                        </span>
                                      );
                                    })()}
                                  </div>
                ...
                {!isStandaloneGroupCollapsed && (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 animate-in fade-in duration-200">
                    ...
                    <div
                      ...
                      className={cn(
                        "group bg-white p-3 sm:p-3.5 rounded-2xl border transition-all cursor-pointer relative flex flex-col justify-between",
                        selectedProjectIds.has(project.id)
                          ? "border-indigo-500 ring-2 ring-indigo-500/20 shadow-md"
                          : "border-gray-200/80 shadow-xs hover:shadow-lg hover:border-blue-300",
                      )}
                    >
                      {/* Row 1: Header */}
                      <div>
                        <div className="flex items-start gap-3 mb-2.5">
                          <div className={cn(
                            ...
                            project.status === "sent" ? "bg-orange-50 text-orange-600" : project.status === "transferred" ? "bg-blue-50 text-blue-600" : "bg-blue-50 text-blue-600"
                          )}>
                            <FileText className="w-5 h-5" />
                          </div>
                          <div className="flex-1 min-w-0">
                            <h3 className="font-bold text-gray-900 truncate text-sm mb-0.5">{project.name}</h3>
                            ...
                            {((project as any).contractNumber || project.data?.contractNumber) && (
                              <div className="mt-1 flex items-center gap-1 text-[10px] font-bold text-blue-700 bg-blue-50 px-1.5 py-0.5 rounded-md w-fit">
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <div 
                              ...
                              className={cn(
                                ...
                                selectedProjectIds.has(project.id) ? "bg-indigo-600 border-indigo-600 text-white" : "bg-white border-gray-300"
                              )}>
                              {selectedProjectIds.has(project.id) && <CheckCircle2 className="w-3.5 h-3.5" />}
                            </div>
                          </div>
                        </div>
                        ...
                        {openMenuId === project.id && (
                          <div className="absolute top-12 right-3 w-52 bg-white rounded-2xl shadow-2xl border border-gray-100 py-2 z-[100]" onClick={(e) => e.stopPropagation()}>
                            {isSelectionMode && (
                      ...
                      <div className="flex items-center justify-between pt-2 border-t border-gray-100 mt-2">
                        {(() => {
                          const isCurrentUser = project.createdBy === userId || (currentUser && (project.createdBy === currentUser.uid || project.createdBy === currentUser.id));
                          const employee = companyEmployees.find(emp => 
                            emp.uid === project.createdBy || 
                            emp.id === project.createdBy ||
                            (project.createdByName && (emp.name === project.createdByName || emp.displayName === project.createdByName))
                          );
                          const displayManagerName = employee ? (employee.name || employee.displayName || employee.email) : isCurrentUser ? (currentUser?.displayName || currentUser?.name || currentUser?.email || project.createdByName || "Пользователь") : (project.createdByName || "Пользователь");
                          const avatarLetter = (displayManagerName?.charAt(0) || "U").toUpperCase();
                          const avatarUrl = employee?.photoURL || employee?.avatarUrl || employee?.photoUrl || employee?.avatar || employee?.photo || (isCurrentUser ? (currentUser?.photoURL || currentUser?.avatarUrl || currentUser?.photoUrl || currentUser?.photo) : null) || project.data?.createdByPhoto || project.createdByPhoto;
                          return (
                            <div className="flex items-center gap-1.5 min-w-0">
                              <div className="w-6.5 h-6.5 bg-gray-100 rounded-full flex-shrink-0 flex items-center justify-center text-[10px] font-bold text-gray-600 overflow-hidden ring-1 ring-gray-200 shadow-2xs">
                                {avatarUrl ? (
                                  <img 
                                    src={avatarUrl} 
                                    ...
                                    onError={(e) => {
                                      ...
                                      if (e.currentTarget.parentElement) {
                                        e.currentTarget.parentElement.innerText = avatarLetter;
                                      }
                                    }}
                                  />
                                ) : (
                              ...
                              <div className="flex flex-col min-w-0">
                                <span className="text-[8px] text-gray-400 font-bold uppercase tracking-tight">Менеджер</span>
                                <span className="text-[10px] text-gray-900 font-bold truncate max-w-[100px]" title={displayManagerName}>
                                  {displayManagerName}
                                </span>
                              </div>
                            </div>
