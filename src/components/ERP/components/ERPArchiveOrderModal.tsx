...
export const ERPArchiveOrderModal: React.FC<ERPArchiveOrderModalProps> = ({
  ...
  const stageNames: Record<string, { label: string; color: string }> = {
    ...
    cut: { label: 'Распил', color: 'bg-amber-100 text-amber-800 border-amber-200' },
    edging: { label: 'Кромка', color: 'bg-blue-100 text-blue-800 border-blue-200' },
    edge: { label: 'Кромка', color: 'bg-blue-100 text-blue-800 border-blue-200' },
    ...
    shipping: { label: 'Отгрузка водителю', color: 'bg-emerald-100 text-emerald-800 border-emerald-200' }
  };
              ...
              <div className="bg-white p-5 rounded-2xl border border-slate-200 space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
                  {Object.entries(stageNames).map(([stageKey, meta]) => {
                    // Skip alias keys to avoid visual duplicates in timeline
                    if (['cut', 'edge', 'cnc', 'packing'].includes(stageKey)) return null;

                    const stData = stageProgress[stageKey as keyof typeof stageProgress];
                    let isDone = stData?.status === 'done';
                    let isInProgress = stData?.status === 'in_progress';
                    const forced = forcedStages[stageKey];

                    // Special rule for shipped / completed orders: shipping stage is marked as DONE
                    if (isFullyShipped || order.status === 'shipped' || order.status === 'completed' || !!order.shippedAt) {
                      if (stageKey === 'shipping' || stageKey === 'ready' || stageKey === 'packaging') {
                        isDone = true;
                        isInProgress = false;
                      }
                    }

                    const completedBy = stData?.completedBy || (
                      (stageKey === 'shipping' || stageKey === 'ready') && (isDone || isFullyShipped || order.status === 'shipped')
                        ? (order.shippedByEmployeeName || order.driverInfo?.driverName || 'Экспедитор')
                        : '—'
                    );

                    const completedAt = stData?.completedAt || (
                      (stageKey === 'shipping' || stageKey === 'ready') && (isDone || isFullyShipped || order.status === 'shipped')
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="space-y-1 text-xs text-slate-600">
                          <div>
                            Исполнитель: <strong className="text-slate-900">{completedBy}</strong>
                          </div>
                          <div>
                            Дата завершения: <span className="font-mono text-slate-800">{formatDateTimeSafe(completedAt)}</span>
                          </div>
                          {stData?.notes && (
